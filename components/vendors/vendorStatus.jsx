...
const statusConfig = [
  ...
  { key: "active", label: "Verified", icon: CheckCircle, color: "text-green-600 bg-green-100" },
...
export default function VendorStatus() {
  ...
  if (data?.success && Array.isArray(data.data)) {
    counts.total = data.data.length
    counts.active = data.data.filter((r) => Number(r.is_verified) === 1).length
    counts.pending = data.data.filter((r) => Number(r.is_verified) === 0).length
  }
