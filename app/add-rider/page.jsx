...
const getIconForField = (fieldName) => {
  ...
  if (
    ...
    fieldNameLower.includes("certificate")
  )
...
function TempTenderView() {
  ...
  const sections = {
    ...
    "LOA AWARDED TO SWCL": [
      59, 60, 126, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 105, 104,
      111,
    ],
  };
  ...
  const priceCheckpoints = [
    10, 12, 15, 17, 22, 24, 26, 28, 29, 62, 64, 66, 70, 72,
  ];
      ...
      details.forEach((item) => {
        ...
        if (ids.includes(baseId) && item.Value !== null && item.Value !== "") {
          const label = item.ChkId.includes("_")
            ? `${
                checkpoints[item.ChkId.split("_")[1]] ||
                `Checkpoint #${item.ChkId.split("_")[1]}`
              } (${sectionTitle})`
    ...
    const handleFileChange = async (e) => {
      ...
      try {
        ...
        const response = await axios.post(
          "https://namami-infotech.com/SANCHAR/src/menu/edit_image.php",
          {
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          >
              ...
              {isEditing ? (
                <div style={{ marginBottom: "15px" }}>
                  <FileUploadField
                    ...
                    currentValue={
                      editedData[studentPhotoChkId] ||
                      getValueByChkId(studentPhotoChkId)
                    }
                ...
                <div
                  ...
                  onClick={() =>
                    openFileInNewTab(getValueByChkId(studentPhotoChkId))
                  }
                >
            ...
            >
              <StyledTitle
                level={3}
                style={{ color: "#F69320", marginBottom: "15px" }}
              >
                Key Information
              </StyledTitle>
                    ...
                    {isEditing ? (
                      <>
                        <StyledInput
                          ...
                          placeholder={
                            field.isPdf ? "Enter PDF URL" : "Enter image URL"
                          }
                        />
                        <FileUploadField
                          chkId={field.id}
          ...
          >
            ...
            >
              {sectionData.map((item, index) => {
                ...
                const isPriceField = priceCheckpoints.includes(
                  Number(item.ChkId)
                );
                ...
                return (
                  <StyledFieldBox
                    ...
                    style={{
                      backgroundColor:
                        isImage || isPdf
                          ? "rgba(246, 147, 32, 0.05)"
                          : undefined,
                      border:
                        isImage || isPdf
                          ? "1px solid rgba(246, 147, 32, 0.2)"
                          : undefined,
                    }}
                  >
                    <StyledFieldLabel>
                        ...
                        {isImage || isPdf ? (
                          <>
                            <StyledInput
                              ...
                              onChange={(e) =>
                                handleInputChange(item.ChkId, e.target.value)
                              }
                              placeholder={
                                isPdf
                                  ? "Enter PDF URL"
                            ...
                            <FileUploadField
                              chkId={item.ChkId}
                              currentValue={
                                editedData[item.ChkId] || item.Value
                              }
                              onChange={handleInputChange}
                              isEditing={isEditing}
                            />
                          ...
                          <StyledDateInput
                            value={editedData[item.ChkId] || ""}
                            onChange={(value) =>
                              handleInputChange(item.ChkId, value)
                            }
                          />
                        ) : (
                          <StyledInput
  ...
  return (
    <StyledContainer>
      ...
      <StyledHeader>
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          ...
          <div>
            ...
            <StyledSubtitle style={{ margin: 0 }}>
              TENDER Form Details{" "}
              {isAdmin && (
                <StyledBadge color="#4caf50">Admin Access</StyledBadge>
              )}
            </StyledSubtitle>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
          <StyledButton
            onClick={cycleFieldVariant}
            style={{ display: "flex", alignItems: "center", gap: "5px" }}
          >
            <Filter size={16} />
            Style:{" "}
            {fieldVariant.charAt(0).toUpperCase() + fieldVariant.slice(1)}
          </StyledButton>
